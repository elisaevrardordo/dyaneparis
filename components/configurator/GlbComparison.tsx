...
function cloneForComparison(source: THREE.Group, mode: ComparisonMode) {
  ...
  const neutral = new THREE.MeshStandardMaterial({ color: '#bdb8b1', roughness: 0.62, metalness: 0 })
  ...
  clone.traverse((object) => {
    ...
    else if (name.includes('body.001')) object.material = signature
...
function ComparisonScene({ mode, orientation }: { mode: ComparisonMode; orientation: Orientation }) {
  ...
  return (
    <>
      ...
      <ComparisonModel source={original.scene} x={-horizontalOffset} mode={mode} orientation={orientation} />
      <ComparisonModel source={v2.scene} x={horizontalOffset} mode={mode} orientation={orientation} />
    ...
    <main className={styles.page}>
      ...
      <section className={styles.comparison} aria-label="Comparaison 3D synchronisée">
        ...
        <div className={`${styles.modelLabel} ${styles.leftLabel}`}>
          <span>Original</span>
          <strong>{metrics.original.triangles.toLocaleString('fr-FR')} triangles</strong>
          <small>{formatWeight(metrics.original.bytes)} · {metrics.original.drawCalls} draw calls</small>
        </div>
        <div className={`${styles.modelLabel} ${styles.rightLabel}`}>
          <span>Dyane Web V2</span>
          <strong>{metrics.v2.triangles.toLocaleString('fr-FR')} triangles</strong>
          <small>{formatWeight(metrics.v2.bytes)} · {metrics.v2.drawCalls} draw calls</small>
        </div>
        <Canvas
          shadows
          dpr={[1, 1.7]}
        ...
        >
          <Suspense fallback={null}>
            <ComparisonScene mode={mode} orientation={orientation} />
          </Suspense>
        </Canvas>
      </section>
